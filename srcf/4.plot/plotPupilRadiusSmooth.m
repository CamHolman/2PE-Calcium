function fig = plotPupilRadiusSmooth(majorR, windowWidth)
    %{
        Introduction
            Plot mousePupil radius per second with smoothing from moving average window.

                Note: In order to avoid artifacts that arrise at t = 0 from moving window 
                averaging, data of length windowWidth is reflected across t = 0 

        Inputs:
           majorR       |   Cell array generated as the output of readpupil
                              
                              Note: do not pass a vector, pass the cell array
                              generated by readpupil. Conversion to a vector 
                              with cell2mat without handeling Nans will cause 
                              dropping of frames. Conversion and interpolation 
                              handled here.


            windowWidth |   Width of window for calculating moving average. For the sampling
                            rate of 30 FPS, a width of 101 frames appeared to work well. Make 
                            the window width an odd number
        Outputs:
    ...
    %{
        ...
        Alternatively, add parameters function 
    %}
